...
def evaluate_scheduled_compliance(event, configuration_item, valid_rule_parameters):
    evaluations = []
    ...
    while True:
        ...
        for role in roles_list['Roles']:
            ...
            if len(role_policies_inline)+len(role_policies_managed) == 0 :
                evaluations.append(build_evaluation(role['RoleId'], 'NON_COMPLIANT', event))
                continue

            evaluations.append(build_evaluation(role['RoleId'], 'COMPLIANT', event))
...
def evaluate_parameters(rule_parameters):
    if not rule_parameters:
        ...
        raise ValueError("This Config rule does not take any parameters")
